...
import {addMediaToUser, calculateTotalAge, filterUsersByLocation} from "../src/user/user-utils";
let mockUser:User;
let mockUser1:User;
let mockUser2:User;
let mockUser3:User;
...
describe("add Media To User", () => {
    ...
    it("should add the Profile picture to the Profile of the user", () => {
        expect(addMediaToUser(mockUser, mockMedia)).
        toEqual(mockUser);
    });
...
describe("filter Users By Location", () => {
    ...
    it("should return all Users at the same location", () => {
        expect(filterUsersByLocation(userArr,"London")).
        toEqual([mockUser2,mockUser3]);
    });

    it("should return noone if location has no users", () => {
        expect(filterUsersByLocation(userArr,"Österreich")).
        toEqual([]);
    });
